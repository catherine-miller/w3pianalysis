...
//TEST functions
bool isolation(int pidex, Vec<float> eta, Vec<float> phi, Vec<float> pt, float mindr, float maxdr, float maxiso) {
    bool passed=false;
    float psum = 0;
...
pitriplet maketriplets(Vec<int> pdgids, Vec<int> charges, Vec<float> pts, Vec<float> etas, Vec<float> phis, Vec<float> masses) {
    Vec<pitriplets> triplets;
   ...
   if (ix.size() > 2) { //if there are 3+ pions
                        ...
                        if (mass >= minmass and mass <= maxmass) { //MASS test
                            if (deltar(p1, p2, p3, mindeltar) == true) { //ANGULAR SEPARATION test
                                ...
                                for (int j = 0; j < tr.size(); ++j) {
                                    auto isopos_ = find(isoindex.begin(), isoindex.end(), tr[j]);
                                    if (isopos_ != isoindex.end()) { //check if isolation has already been calculated
                                        ...
                                        isoindex.push_back(tr[j]); //add particle index to isolation index list
                                        if (isolation(tr[j], etas, phis, pts, mindr, maxdr, maxiso) == true) {
    ...
    for (std::size_t i = 0; i < triplets.size(); ++i) {
        float score = triplets[i].mass*ptsums[i];
        if (score > bestscore) {
            bestscore = score;
            best = i;
        }
