...
auto maketriplets(Vec<int> pdgids, Vec<int> charges, Vec<float> pts, Vec<float> etas, Vec<float> phis, Vec<float> masses) {
    ...
    Vec<Vec<std::size_t>> drs(pdgids.size(), Vec<std::size_t>(pdgids.size(),0)); //stores drs so that we don't calculate twice
   ...
   if (ix.size() > 2) { //if there are 3+ pions
       ...
       for (std::size_t i1 = 0; i1 < ix.size(); ++i1) {
           ...
           for (std::size_t i2 = 0; i2 < ix.size(); ++i2) {
               ...
               if (drs[ix[i1]][ix[i2]] == 0) {
                    if (deltar(ix[i1], ix[i2], etas[ix[i1]], etas[ix[i2]], phis[ix[i1]], phis[ix[i2]], mindeltar) == false) {
                        drs[ix[i1]][ix[i2]] = 2;
                        continue;
                    }
                    else drs[ix[i1]][ix[i2]] = 1;
               }
               else if (drs[ix[i1]][ix[i2]] == 2) break;
                   ...
                   if (pts[ix[i3]] > pts[ix[i2]] || (pts[ix[i3]] == pts[ix[i2]] and i3 < i2)) continue;
                    ...
                    if (abs(charges[tr[0]] + charges[tr[1]] + charges[tr[2]]) == 1) {
                        ...
                        if (mass >= minmass and mass <= maxmass) { //MASS test
                            //Angular separation test
                            if (drs[ix[i1]][ix[i3]] == 0) {
                                if (deltar(ix[i1], ix[i3], etas[ix[i1]], etas[ix[i3]], phis[ix[i1]], phis[ix[i3]], mindeltar) == true) drs[ix[i1]][ix[i3]] = 1;
                                else {
                                    drs[ix[i1]][ix[i3]] = 2;
                                    continue;
                                }
                            }
                            else if (drs[ix[i1]][ix[i3]] == 2) continue;
                            if (drs[ix[i2]][ix[i3]] == 0) {
                                if (deltar(ix[i2], ix[i3], etas[ix[i2]], etas[ix[i3]], phis[ix[i2]], phis[ix[i3]], mindeltar) == true) drs[ix[i2]][ix[i3]] = 1;
                                else {
                                    drs[ix[i2]][ix[i3]] = 2;
                                    continue;
                                }
                            }
                            else if (drs[ix[i2]][ix[i3]] == 2) continue;
                            //ISOLATION test
                            bool isop = true;
                            for (int j = 1; j < 3; ++j) {
                                if (iso[tr[j]] == 0) {
